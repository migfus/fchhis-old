...
class UserController extends Controller
{
  ...
  private function ValidationResponse($req, $role) {
  ...
  public function SearchResponse($req, $user) {
    switch($req->filter) {
      case 'name':
        $user->with('person')
    ...
    if($req->user()->role == 2) {
      ...
      if((bool)strtotime($req->start) OR (bool)strtotime($req->end)) {
        ...
        return response()->json([
          ...
          'data' => $user->with('person')->paginate(10)
        ]);
      }
      ...
      if($req->role < 7) {
        ...
        $this->SearchResponse($req, User::select('*'));
      }
    }
    // return $this->UnauthorizedResponse();
  }
